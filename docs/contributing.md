...
本项目使用 [Docusaurus](https://docusaurus.io/zh-CN/) v3 构建静态文档网站。
...
1. **克隆仓库**：
   ...
   ```bash
   git clone https://github.com/rurumuri/ysuse-2022.git
   cd ysuse-2022
   ```
...
3. **构建和启动**：
   ...
   开发服务器默认运行在 `http://localhost:3000`。任何对 Markdown 文件、配置或组件的修改都会触发热重载，浏览器会自动刷新。
...
本仓库已启用 `git-lfs`，符合 `.gitattributes` 中配置的文件会以 LFS 方式存储。请避免上传大尺寸二进制文件，上传项目代码时请注意清理构建产物。
