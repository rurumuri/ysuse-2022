...
// 启发函数
template<class T>
T arrayWDigraph<T>::_aStarHeuristic(int current, int goal) {
    // 这里简单地使用节点编号的差值作为启发式值
    return abs(current - goal);
}

// A*算法
template<class T>
vector<int> arrayWDigraph<T>::_aStar(int start, int end) {
    // 定义结点信息
    struct AStarNode {
        int node_id;    // 节点编号
        T g_cost;       // 从起始节点到当前节点的实际代价
        T h_cost;       // 从当前节点到目标节点的估计代价（启发式值）
        T f_cost;       // f(n) = g(n) + h(n)，表示节点的优先级
    };

    // 定义优先级队列，使用 lambda 表达式作为比较函数
    auto cmp = [](const AStarNode& a, const AStarNode& b) {
        return a.f_cost > b.f_cost; // 注意这里是大于号，因为优先级队列默认是大顶堆
    };
    priority_queue<AStarNode, std::vector<AStarNode>, decltype(cmp)> pq(cmp);

    // 存储已访问的节点
    vector<int> visited(n_vertex + 1, 0);

    // 存储父节点，用于回溯路径
    vector<int> parent(n_vertex + 1, -1);

    // 初始化起始节点
    AStarNode startNode = {start, 0, _aStarHeuristic(start, end), _aStarHeuristic(start, end)};
    pq.push(startNode);

    while (!pq.empty()) {
        // 弹出优先级最小的节点
        AStarNode currentNode = pq.top();
        pq.pop();

        int currentID = currentNode.node_id;
        ...
        // 遍历当前节点的邻居
        for (const auto& neighbor : G[currentID].conn_vertices) {
            int neighborID = neighbor.node_id;
            if (!visited[neighborID]) {
                // 更新节点信息
                T g_cost = currentNode.g_cost + neighbor.node_weight;
                T h_cost = _aStarHeuristic(neighborID, end);
                T f_cost = g_cost + h_cost;

                // 将邻居节点加入优先级队列
                pq.push({neighborID, g_cost, h_cost, f_cost});
...
// 此为前端函数，检查和提示作用
template<class T>
void arrayWDigraph<T>::AStarGetPath(){
    system("cls");
    cout << "=======Single Vertex Shortest Path (A*)=======" << endl;
    if (n_vertex == 0){
        cout << "Error: Graph empty" << endl;
        system("pause");
        return;
    ...
    if (node_id_start == node_id_end){
        ...
        // out
        cout << "Path: ";
        auto res = _aStar(node_id_start, node_id_end);
        for(auto i : res){
            cout << i;
            if(i != res.back()) cout << "->";
        }
...
template<class T>
void arrayWDigraph<T>::_shortPathVia(int start, int through, int end) {
    // 调用Dijkstra算法计算最短路径
    T* distanceFromStart = _shortPathDijkstra(start, end);
    T* distanceThroughPoint = _shortPathDijkstra(start, through);
    T* distanceToEnd = _shortPathDijkstra(through, end);

    // 检查路径是否存在
    if (!distanceFromStart || !distanceThroughPoint || !distanceToEnd) {
        cout << "Path not exists" << endl;
        return;
    }

    // 计算总距离
    T totalDistance = distanceThroughPoint[getIndex(through)] + distanceToEnd[getIndex(end)];

    // 输出结果
    cout << endl;
    cout << "Path" << V[start].node_id << " to " << V[end].node_id << " via " << V[through].node_id
         << ": " << totalDistance << endl;

    cout << V[start].node_id << "->" << V[through].node_id << "->" << V[end].node_id << endl;
}

template<class T>
void arrayWDigraph<T>::shortPathVia(){
    system("cls");
    cout << "=======Single Vertex Shortest Path (A*)=======" << endl;
    if (n_vertex == 0){
        cout << "Error: Graph empty" << endl;
    ...
    cout << "Enter the via node_id" << endl;
    while (true) {
        cout<<"> "; cin >> node_id_via;
        if (!existVertex(node_id_via)){
            cout << "Via node not exists! Try again. " << endl;
        }
        else{
            break;
        }
    ...
    if (node_id_start == node_id_end){
        ...
        _shortPathVia(node_id_start, node_id_via, node_id_end);
        system("pause");
    }
}